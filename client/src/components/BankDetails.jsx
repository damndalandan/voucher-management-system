...
const BankDetails = ({ account, user, onUpdate, showAlert }) => {
  ...
  const fetchTransactions = async (id) => {
    try {
      const res = await axios.get(`http://localhost:5000/api/banks/${id}/transactions`);
  ...
  const fetchCheckbooks = async (id) => {
    try {
      const res = await axios.get(`http://localhost:5000/api/banks/${id}/checkbooks`);
  ...
  const fetchChecks = async (id) => {
    try {
      const res = await axios.get(`http://localhost:5000/api/banks/${id}/checks`);
      setChecks(res.data);
    } catch (err) {
      console.error(err);
    }
  ...
  const handleAddCheckbook = async (e) => {
    e.preventDefault();
    try {
      await axios.post(`http://localhost:5000/api/banks/${account.id}/checkbooks`, checkbookForm);
  ...
  const handleTransaction = async (e) => {
    e.preventDefault();
    try {
      await axios.post(`http://localhost:5000/api/banks/${account.id}/transaction`, transactionForm);
  ...
  return (
    <div className="space-y-8 animate-fade-in">
      {/* Header */}
      <div className="bg-gradient-to-r from-blue-600 to-blue-800 p-8 rounded-2xl shadow-lg text-white relative overflow-hidden">
        ...
        <div className="relative z-10 flex flex-col md:flex-row justify-between items-start md:items-center gap-6">
            <div>
                <div className="flex items-center gap-3 mb-2">
                    <h2 className="text-3xl font-bold tracking-tight">{account.bank_name}</h2>
            ...
            <div className="flex gap-3 w-full md:w-auto">
                <button 
                    ...
                    className="flex-1 md:flex-none bg-white text-green-700 px-6 py-3 rounded-xl hover:bg-green-50 font-bold shadow-lg shadow-black/10 flex items-center justify-center gap-2 transition-all transform hover:-translate-y-1"
                >
                    <div className="bg-green-100 p-1 rounded-full"><ArrowDownLeft size={18} /></div>
                    Deposit
                </button>
                <button 
                    onClick={() => {
                    ...
                    className="flex-1 md:flex-none bg-white/10 backdrop-blur-md border border-white/20 text-white px-6 py-3 rounded-xl hover:bg-white/20 font-bold flex items-center justify-center gap-2 transition-all"
                >
                    <div className="bg-white/20 p-1 rounded-full"><ArrowUpRight size={18} /></div>
                    Withdraw
                </button>
            </div>
        </div>
        ...
        <div className="space-y-6">
            ...
            <div className="bg-white rounded-xl shadow-sm border border-gray-100 overflow-hidden">
                <div className="px-6 py-4 border-b border-gray-100 bg-gray-50/50 flex flex-col md:flex-row justify-between items-center gap-4">
                    <h3 className="text-lg font-bold text-gray-800">Issued Checks</h3>
                    <div className="flex items-center gap-3 w-full md:w-auto">
                        ...
                        <input 
                            type="text" 
                            placeholder="Search check no, payee..." 
                            className="bg-white border border-gray-200 text-gray-700 text-sm rounded-lg focus:ring-blue-500 focus:border-blue-500 block p-2 w-full md:w-64"
                            value={searchTerm}
                            onChange={e => setSearchTerm(e.target.value)}
                        />
                        <div className="text-sm text-gray-500 whitespace-nowrap">Total: {filteredChecks.length}</div>
                    </div>
                </div>
                <div className="overflow-x-auto">
                    <table className="min-w-full divide-y divide-gray-200">
                        <thead className="bg-gray-50">
                            <tr>
                                <th className="px-6 py-3 text-left text-xs font-bold text-gray-500 uppercase tracking-wider">Date Issued</th>
      ...
      {activeTab === 'passbook' && (
        <div className="bg-white rounded-xl shadow-sm border border-gray-100 overflow-hidden">
            <div className="px-6 py-4 border-b border-gray-100 bg-gray-50/50 flex flex-col md:flex-row justify-between items-center gap-4">
                <h3 className="text-lg font-bold text-gray-800">Passbook Transaction History</h3>
                <div className="flex items-center gap-3 w-full md:w-auto">
                    <input 
                        type="date" 
                        className="bg-white border border-gray-200 text-gray-700 text-sm rounded-lg focus:ring-blue-500 focus:border-blue-500 block p-2"
                    ...
                    <input 
                        ...
                        onChange={e => setSearchTerm(e.target.value)}
                    />
                    <div className="text-sm text-gray-500 whitespace-nowrap">Total: {filteredTransactions.length}</div>
                </div>
            </div>
            <div className="overflow-x-auto">
                <table className="min-w-full divide-y divide-gray-200">
